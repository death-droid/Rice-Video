...
void SmoothFilter(uint32 *pdata, uint32 width, uint32 height, uint32 pitch, uint32 filter, bool bPixelSize4)
{
	if(bPixelSize4)
		SmoothFilter_32((uint32*)pdata, width<<1, height<<1, pitch<<1, filter);
	else
		SmoothFilter_16((uint16*)pdata, width<<1, height<<1, pitch<<1, filter);
}