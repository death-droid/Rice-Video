...
void DecodedMux::SplitComplexStages()
{
	for( int i=0; i<2; i++)	// Color channel and alpha channel
	{
		...
		case CM_FMT_TYPE_A_MOD_C_ADD_D:		// = A*C+D		can mapped to D3D MULTIPLYADD(arg1,arg2,arg0)
			...
			break;
		case CM_FMT_TYPE_A_SUB_B_ADD_D:		// = A-B+D		can not map very well to D3D in 1 stage
			m2.a = m.d;
			m2.d = MUX_COMBINED;
			m2.c = MUX_1;
			m2.b=0;
		...
		case CM_FMT_TYPE_A_SUB_B_MOD_C:		// = (A-B)*C	can not map very well to D3D in 1 stage
			...
			splitType[i] = CM_FMT_TYPE_A_SUB_B;
			break;
		case CM_FMT_TYPE_A_ADD_B_MOD_C:		// = (A+B)*C	can not map very well to D3D in 1 stage
			m2.a = m.c;
			m2.c = MUX_COMBINED;
			m2.d = m2.b = 0;
			splitType[i+2] = CM_FMT_TYPE_A_MOD_C;
		...
		case CM_FMT_TYPE_A_B_C_D:			// = (A-B)*C+D	can not map very well to D3D in 1 stage
			...
			splitType[i] = CM_FMT_TYPE_A_SUB_B_MOD_C;
			break;
		case CM_FMT_TYPE_A_B_C_A:			// = (A-B)*C+A	can not map very well to D3D in 1 stage
			m2.a = m.d;
			m2.d = MUX_COMBINED;
			m2.c = MUX_1;
			m2.b = 0;
