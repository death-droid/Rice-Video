...
#ifdef _BMG_LIBJPEG_STANDALONE
	#include "BMGLibJPEG.h"
#else
	#include "jpegrw.h"
#endif
#include "BMGUtils.h"
...
#ifdef _BMG_LIBJPEG_STANDALONE
#pragma message ("Creating BMGLibJPEG functions")

/* saves the contents of an HBITMAP to a file.  returns 1 if successfull,
// 0 otherwise */
BMGError SaveBitmapToJPEGFile( HBITMAP hBitmap,      /* bitmap to be saved */
                               ...
                               int quality )
{
	...
	// for more details) */
        ...
        if (  out == BMG_OK )
        {
            out = WriteJPEG( imagename, img, quality );
        }
		else
		{
			SetLastBMGError( out );
...
HBITMAP CreateBitmapFromJPEGFile( const char *filename )
{
    ...
    if ( strcmp( ext, "JPG" ) == 0 )
    {
		BMGError err = ReadJPEG( imgname, &img );
        if ( err == BMG_OK )
		{
