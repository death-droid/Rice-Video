...
void DLParser_RS_Color_Buffer(MicroCodeCommand command)
{
	...
	Rogue_Squadron_Vtx_Color_Cmd = (command.inst.cmd0);
...
void DLParser_RS_Vtx_Buffer(MicroCodeCommand command)
{
	uint32 dwPC = gDlistStack[gDlistStackPointer].pc-8;
	uint32 dwAddr = RSPSegmentAddr((command.inst.cmd1));
	if( dwAddr > g_dwRamSize )
	...
	Rogue_Squadron_Vtx_XYZ_Cmd = (command.inst.cmd0);
...
void DLParser_RS_0xbe(MicroCodeCommand command)
{
	uint32 dwPC = gDlistStack[gDlistStackPointer].pc-8;
	...
	DEBUGGER_PAUSE_AND_DUMP(NEXT_SET_MODE_CMD, {
		...
		DebuggerAppendMsg("\t0x%08x 0x%08x", (command.inst.cmd0), (command.inst.cmd1));
